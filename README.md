...
| 能力 | Read On Bush | Moreover Reader |
|---|---|---|
| 隐藏时屏幕表现 | 正文行直接消失，行数变少、布局跳动 | **等行数伪装待办**顶替正文位置，整屏高度不变、**零抖动** |
| 注释/脚注 | 不支持 | **`e` 键展示**：当前窗口内所有 `[n]` 标记的注解逐条列出，按标记号 / EPUB noteref 精确匹配内容（支持 Calibre 中文书常见结构、上标数字、①–⑳ 圈号） |
| 书籍格式 | 仅 TXT | **EPUB + TXT**（EPUB 零依赖自包含解析：章节、插图、注解） |
| 插图 | 不支持 | **`i` 键预览**最近插图（3 秒自动关，可开关、可常驻） |
| 伪装素材 | 简单状态行 | 假需求分析 + **彩色 diff**（红删绿增青 hunk）+ 测试通过记录，翻页时上下文不变只有正文动 |
| 进度记忆 | 有 | 有，且按书**按项目**记忆，右键移除带确认 |
